...
def generate_message_advice(message: str, classification: str, language_code: str = "en") -> List[Dict]:
    """Generates specific defense advice for a scanned message using Groq."""
    ...
    prompt = f"""
    You are a cybersecurity expert. Analyze this specific message categorized as '{classification}':
    
    "{message}"
    
    Provide exactly 3 actionable defense advice points or security strategies for a user receiving this type of message.
    Language: {lang_name} (code: {language_code})
    
    The output must be a STRICTLY VALID JSON object with:
    - "advice": array of exactly 3 objects, each with:
        - "title" (string, short punchy title in {lang_name})
        - "detail" (string, 1-2 sentence detailed instruction or explanation in {lang_name})
    
    Output ONLY THE JSON OBJECT. No markdown, no preface.
    """
    ...
    try:
        ...
        print(f"Error generating message advice with Groq: {e}")
        return []
